...
async function getCsvUrl(details){
  try{
    const response = await validateRequest(details)
    if(response == true){
      const user_data = await User.findOne({emailId:details.emailId})
      const csv_data = await Csv.find({uuid:String(user_data._id),type:"merchant"}).sort({creation_date:-1}).skip(details.skip).limit(details.limit)
      return mapper.responseMappingWithData(
        usrConst.CODE.Success,
        usrConst.MESSAGE.Success,
        csv_data
