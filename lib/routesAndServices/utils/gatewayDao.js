...
const GatewaySettlements = require('../../generic/models/getwaySettlements'); // Import your GatewayData model
...
async function getGatewayData(gatewayName) {
...
async function getGatewaySettlementsData(details)
{
    try {
        // Get the current date in the format "YYYY-MM-DD" in the Asia/Kolkata timezone
        ...
        const gatewayInfo = await getGatewayData(details.gatewayName);
        const documents = await GatewaySettlements.GatewaySettlements.find({
...
async function createGatewaySettlement(details) {
    try {
        // Define the new settlement data
        const newSettlement = new GatewaySettlements.GatewaySettlements(details);

        // Save the new settlement document to the database
        const savedSettlement = await newSettlement.save();
        console.log('New settlement created:', savedSettlement);

        return savedSettlement; // Return the newly created settlement document
    } catch (error) {
        console.error('Error creating new settlement:', error);
        throw error; // Re-throw the error to handle it further if needed
    }
}

async function updateGatewayDataFields(gatewayName, updateObj) {
    try {
        // Add the `updatedAt` field to track when the document was last updated
        updateObj.updatedAt = new Date();

        // Use findOneAndUpdate to update the fields specified in updateObj
        const updatedDocument = await GatewayModel.Gateway.findOneAndUpdate(
        ...
        if (updatedDocument) {
            ...
            console.log(`No document found with gatewayName: ${gatewayName}`);
            return null;
        }
    } catch (error) {
        console.error('Error updating the gateway settlement:', error);
        throw error; // Re-throwing the error to handle it further if needed
    }
}
