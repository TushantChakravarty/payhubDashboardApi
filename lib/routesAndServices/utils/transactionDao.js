...
function settlementToCsvRow(transaction,name='') {
  ...
  return `${formattedTime},${formattedDate},${name || ''},${transaction.amount !== undefined ? transaction.amount.toFixed(2) : '0.00'},${transaction.feeCharged !== undefined ? transaction.feeCharged.toFixed(2) : '0.00'},${transaction.netBankFees !== undefined ? transaction.netBankFees.toFixed(2) : '0.00'},${transaction.netGst !== undefined ? transaction.netGst.toFixed(2) : '0.00'},${transaction.chargeback !== undefined ? transaction.chargeback.toFixed(2): '0.00'},${transaction.rolling_reserve !== undefined ? transaction.rolling_reserve.toFixed(2) : '0.00'},${transaction.misc !== undefined ? transaction.misc.toFixed(2) : '0.00'},${transaction.refund !== undefined ? transaction.refund.toFixed(2) : ''},${transaction.amountSettled !== undefined ? transaction.amountSettled.toFixed(2) : '0.00'},${transaction.usdtRate !== undefined?transaction.usdtRate.toFixed(2) : '0.00'},${transaction.usdt !== undefined ? transaction.usdt.toFixed(2) : '0.00'},${transaction.ref_no || ''},${transaction.notes || ''}\n`;
...
//     // CSV header
//     passThroughStream.write("Time,Date,Merchant Name,Gross Volume ,Net Fees,Bank Fees,Net Volume,USDT Rate,USDT Net,Ref,Notes,Net gst,Gst,Refund,Charge back,Rolling reserve,Misc\n");

//     cursor.on('data', (doc) => {
//       // Convert the document to a CSV row and write to the stream
//       passThroughStream.write(settlementToCsvRow(doc,data.name));
...
//       passThroughStream.write(`,,TOTALS,${totalGrossVolume.toFixed(2)},${totalNetFees.toFixed(2)},${totalBankFees.toFixed(2)},${totalNetVolume.toFixed(2)},${averageUsdtRate},${totalUsdtNet.toFixed(2)},,,,,,,,\n`);
