...
const PayhubModel = require("../../generic/models/payhubData")
const fs = require('fs');
...
async function uploadCsvStreamEmailReport(readableStream) {
  const params = {
    Bucket: process.env.S3_BUCKET_NAME, // Your bucket name
    Key: `payhub_report_${Date.now()}.csv`, // File name to save in S3
    Body: readableStream, // Streaming the CSV data
    ContentType: 'text/csv',
  };
...
async function uploadCsvStreamEmailReportNew(readableStream) {
  const params = {
    Bucket: process.env.S3_BUCKET_NAME, // Your bucket name
    Key: `payhub_report_${Date.now()}.xlsx`, // File name to save in S3
    Body: readableStream, // Streaming the CSV data
    ContentType: 'text/csv',
  };
...
function emailReportCsvRow(data,type) {
  try{
    if(type == "admin"){
       return `${data.business_name},${data.yesterday}\n`
    }else if( type == "merchant"){
      return `${data.business_name},${data.yesterday}\n`
    }else if(type == "gateway"){
      return `${data.gatewayName},${data.yesterday}\n`
    }
  }catch(error){
    console.log(error)
    return false
...
async function downloadEmailReport() {
  try {
    const payhub_data = await PayhubModel.find({})
    const yesterday_payhub_volume = payhub_data[0].yesterday
   ...
   passThroughStream.write("PAYHUB\nNAME,TOTAL 24 HOUR\n");
   passThroughStream.write(`PAYHUB,${yesterday_payhub_volume}\n`)

   const merchant_data = await User.find({}).sort({ yesterday: -1 });
   passThroughStream.write("\n");
   passThroughStream.write("MERCHANTS\nNAME,TOTAL 24 HOUR\n")
   merchant_data.map((item)=>{
    if(item.business_name !== "" && item.business_name !== undefined){
      passThroughStream.write(`${item.business_name || 'NA'},${item.yesterday || 0}\n`)
    }
 
   })
   passThroughStream.write("\n");
   passThroughStream.write("GATEWAY\nNAME,TOTAL 24 HOUR\n")
   const gateway_data = await Gateway.find({}).sort({ yesterday: -1 });
   gateway_data.map((item)=>{
    passThroughStream.write(`${item.gatewayName || ''},${item.yesterday || 0}\n`)
   })
 

  passThroughStream.end()
  const csvUrl = await uploadCsvStreamEmailReport(passThroughStream);

   return {url:csvUrl,payhub_data:yesterday_payhub_volume,merchant_data,gateway_data}
  } catch (error) {
    console.error("Error:", error);
...
//     // const payhub_data = await PayhubModel.find({});
//     // const yesterday_payhub_volume = payhub_data[0].yesterday;
//     // const merchant_data = await User.find({});
//     // const gateway_data = await Gateway.find({});
...
//     // // Add a header row
//     // sheet.addRow(['TYPE: PAYHUB', 'NAME', 'TOTAL 24 HOUR']);
//     // sheet.addRow(['PAYHUB', yesterday_payhub_volume]);

//     // // Adding an empty row for spacing
//     // sheet.addRow([]);

//     // // Merchant data
//     // sheet.addRow(['TYPE: MERCHANTS', 'NAME', 'TOTAL 24 HOUR']);
//     // merchant_data.forEach(item => {
//     //   sheet.addRow([item.business_name || '', item.yesterday || 0]);
//     // });
...
//     // // Gateway data
//     // sheet.addRow(['TYPE: GATEWAY', 'NAME', 'TOTAL 24 HOUR']);
//     // gateway_data.forEach(item => {
//     //   sheet.addRow([item.gatewayName || '', item.yesterday || 0]);
//     // });
...
//     // // Define a common bold style
//     // const boldStyle = { font: { bold: true } };

//     // // Add and style header for Payhub
//     // sheet.mergeCells('A1:A2');
//     // sheet.getCell('A1').value = 'TYPE: PAYHUB';
//     // sheet.getCell('A1').font = boldStyle.font;
//     // sheet.getCell('A1').alignment = { vertical: 'middle', horizontal: 'center' };
//     // sheet.addRow(['NAME', 'TOTAL 24 HOUR']).commit();
//     // sheet.getCell('B2').value = 'PAYHUB';
//     // sheet.getCell('C2').value = yesterday_payhub_volume;

//     // // Spacer
//     // sheet.addRow([]);

//     // // Merchant data header
//     // const merchantStartRow = sheet.lastRow.number + 1;
//     // sheet.mergeCells(`A${merchantStartRow}:A${merchantStartRow + merchant_data.length}`);
//     // sheet.getCell(`A${merchantStartRow}`).value = 'TYPE: MERCHANTS';
//     // sheet.getCell(`A${merchantStartRow}`).font = boldStyle.font;
//     // sheet.getCell(`A${merchantStartRow}`).alignment = { vertical: 'middle', horizontal: 'center' };

//     // merchant_data.forEach(item => {
//     //   sheet.addRow([item.business_name || '', item.yesterday || 0]).commit();
//     // });
...
//     // // Gateway data header
//     // const gatewayStartRow = sheet.lastRow.number + 1;
//     // sheet.mergeCells(`A${gatewayStartRow}:A${gatewayStartRow + gateway_data.length}`);
//     // sheet.getCell(`A${gatewayStartRow}`).value = 'TYPE: GATEWAY';
//     // sheet.getCell(`A${gatewayStartRow}`).font = boldStyle.font;
//     // sheet.getCell(`A${gatewayStartRow}`).alignment = { vertical: 'middle', horizontal: 'center' };

//     // gateway_data.forEach(item => {
//     //   sheet.addRow([item.gatewayName || '', item.yesterday || 0]).commit();
//     // });
...
//     const headerRow = sheet.addRow(['TYPE: PAYHUB', 'NAME', 'TOTAL 24 HOUR']);
...
//     // Merchant data header
//     const merchantHeader = sheet.addRow(['TYPE: MERCHANTS', 'NAME', 'TOTAL 24 HOUR']);
//     merchantHeader.eachCell((cell) => {
//       cell.font = boldStyle.font;
//     });
...
//     // Gateway data header
//     const gatewayHeader = sheet.addRow(['TYPE: GATEWAY', 'NAME', 'TOTAL 24 HOUR']);
//     gatewayHeader.eachCell((cell) => {
//       cell.font = boldStyle.font;
//     });
...
//     // Upload buffer to S3
//     const csvUrl = await uploadCsvStreamEmailReportNew(buffer);
//     return csvUrl;
//   } catch (error) {
//     console.error("Error:", error);
//     throw new Error("Error from download settlement csv by date");
...
async function downloadGatewaySettlementTransactionsByDateWithStatusCsv(query) {
  try {
    // Convert startDate and endDate to IST
    console.group(query.startDate, query.endDate)
    const istStartDate = moment.tz(query.startDate, "Asia/Kolkata").startOf("day").toISOString();
    const istEndDate = moment.tz(query.endDate, "Asia/Kolkata").endOf("day").toISOString();
