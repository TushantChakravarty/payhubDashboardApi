...
function settlementToCsvRow(transaction,name='') {
  ...
  return `${formattedTime},${formattedDate},${name || ''},${transaction.amount || ''},${transaction.currency || ''},${transaction.transaction_date || ''},${transaction.netFees || ''},${transaction.bankFees || ''},${transaction.usdtRate || ''},${transaction.usdt || ''},${transaction.ref_no || ''},${transaction.notes || ''},${transaction.netGst || ''},${transaction.gst || ''},${transaction.refund || ''},${transaction.chargeback || ''},${transaction.rolling_reserve || ''},${transaction.misc || ''}\n`;
}
...
async function downloadSettlementTransactionsByDateWithStatusCsv(query) {
  try {
    ...
    // CSV header
    passThroughStream.write("Time,Date,Merchant Name,Net Volume ,Currency ,Tx Date ,Net Fees ,Bank Fees,USDT Rate,USDT Net,Ref,Notes,Net gst,Gst,Refund,Charge back,Rolling reserve,Misc\n");
