...
const { resetPasswordSandbox, resetPasswordSandboxAdmin } = require("../sandbox/sandbox");
                ...
                return dao
                  ...
                  .then(async (userUpdated) => {
                    if (userUpdated) {
                      ...
                      const updated = await resetPasswordSandboxAdmin(sandboxUpdate)
                      console.log("success", userUpdated);
                      updateObj.emailId = userUpdated.emailId;
  ...
  return await validateRequest(details).then(async (response) => {
    ...
    if (response == true) {
      ...
      const updateDetails = {
        ...
        rolling_reserve:details?.rolling_reserve?details?.rolling_reserve:0,
    ...
    if (response == true) {
      ...
      if (details.email_Id == "all") {
        return transactionDao
          .getTransactionsByDate(details.start_date, details.end_date, details.limit, details.skip)
          .then((user) => {
            if (user) {
              //console.log('success', user)
...
async function updateGatewaySettlements(details) {
  return validateRequest(details).then(async (response) => {
    if (response == true) {
      ...
      const settlements = Number(gatewayInfo.settlements) + Number(details.amount);
      const gatewayUpdate = {
        balance,
        settlements,
      };
...
async function addTopup(req, res) {
  try {
    ...
    const selectedFields = ['_id', 'emailId', 'business_name','payoutBalance','topups'];
   ...
   let admin = await Admin.findOne({emailId:req.body.emailId,apiKey:req.body.apiKey})
   if(admin){
  ...
  if (details.emailId && details.apiKey) {
    return await validateRequest(details).then(async (response) => {
      if (response == true) {
        ...
        const response = await transactionDao.downloadTransactionsCsvByDate(query);
  ...
  if (details.emailId && details.apiKey) {
    return await validateRequest(details).then(async (response) => {
      if (response == true) {
        ...
        const response = await transactionDao.downloadTransactionsCsvByDateAndStatus(query);
  ...
  if (details.emailId && details.apiKey) {
    return await validateRequest(details).then(async (response) => {
      if (response == true) {
        ...
        const response = await transactionDao.downloadAllMerchantTransactionsCsv(query);
  ...
  if (details.emailId && details.apiKey) {
    return await validateRequest(details).then(async (response) => {
      if (response == true) {
        ...
        const response = await transactionDao.downloadAllMerchantTransactionsByStatusCsv(query);
  ...
  if (details.emailId && details.apiKey) {
    return await validateRequest(details).then(async (response) => {
      if (response == true) {
        ...
        const response = await transactionDao.downloadAllTransactionsByStatus(query);
  ...
  if (details.emailId && details.apiKey) {
    return await validateRequest(details).then(async (response) => {
      if (response == true) {
        ...
        const response = await transactionDao.downloadMerchantTransactionsCsvByDate(query);
  ...
  if (details.emailId && details.apiKey) {
    return await validateRequest(details).then(async (response) => {
      if (response == true) {
        ...
        const response = await transactionDao.downloadMerchantTransactionsCsvByDateAndStatus(query);
    ...
    if (details.emailId && details.apiKey) {
      return await validateRequest(details).then(async (response) => {
        if (response == true) {
          ...
          const response = await transactionDao.downloadSettlementTransactionsByDateWithStatusCsv(query);
...
async function downloadGatewaySettlementTransactionsByDateWithStatusCsv(details) {
  try{
    if (details.emailId && details.apiKey) {
      return await validateRequest(details).then(async (response) => {
        if (response == true) {
          ...
          const response = await transactionDao.downloadGatewaySettlementTransactionsByDateWithStatusCsv(query);
  ...
  try{
    ...
    if(details.settlementType === "settlement"){
      ...
      const updateDetails = {
        ...
        feeCharged: details.platformFee ? details.platformFee : 0,
        bankFees:details.bankFee ? details.bankFee : 0,
        gst:details.gst ? details.gst:0, 
        transaction_date: today.toISOString(),
        type:"settlement"
      };
      ...
      let admin_update_details = {
        feeCollected24hr: admin.feeCollected24hr + details?.platformFee,
        totalFeeCollected: admin.totalFeeCollected + details?.platformFee,
        balance: Number(admin.balance) - Number(details.amount),
        totalSettlementAmount: Number(admin.totalSettlementAmount) + Number(details.amount)
      }
    ...
    const admin_updated = await Admin.findOneAndUpdate(adminQuery,update,options)
    ...
    const user_updated = await User.findOneAndUpdate(query,user_update,options)
    return mapper.responseMappingWithData(
      usrConst.CODE.Success,
      usrConst.MESSAGE.Success,
      "Settlement Updated"
      ...
      const bankFees = details.platformFee ? details.platformFee : 0
      const amount = details.amount ? details.amount:0
      if (bankFees < 0){
        return mapper.responseMapping(
          usrConst.CODE.INTRNLSRVR,
