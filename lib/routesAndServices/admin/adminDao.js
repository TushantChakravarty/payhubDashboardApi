...
const { Transaction,PayoutTransaction } = require('../../generic/models/TransactionData'); // Adjust the path accordingly
...
const { getAllTransactionsData, getAllMerchantTransactions } = require("../utils/transactionDao");
const { updateGatewayDetailsNew } = require("../gateway/gatewayDao");
const { updatePayhubDataFields } = require("../utils/payhubDao");
const { getAllGatewayData, getGatewaySettlementsData, updateGatewayDataFields, createGatewaySettlement } = require("../utils/gatewayDao");
...
async function getTransactionByDateWithStatus(userId, startDate, endDate, status,limit, skip) {
  try {
...
async function getTotalPayoutVolumeAndTopups(emailId) {
  ...
  return { totalPayoutVolume: userTransactions[0]?.totalVolume ? userTransactions[0]?.totalVolume : 0, totalTopupVolume: userTopupTransactions[0]?.totalTopupVolume ? userTopupTransactions[0]?.totalTopupVolume : 0 };
}
    ...
    Users.forEach(async (user) => {
      try {
        ...
        if (user) {
          ...
          let totalVolume = await getTotalMerechantVolume(user.emailId, String(user._id), 'success')
...
async function getTotalAdminBalance(status) {
  try {
    ...
    const totalVolume = transactionResult.length > 0 ? transactionResult[0].totalVolume : 0;
    ...
    const totalSettlements = settlementResult.length > 0 ? settlementResult[0].totalAmount : 0;
  ...
  try {
    ...
    let update = {
      $inc: {
        ...
        "gateways.$.feeCollected24hr": updateDetails.$inc?.feeCollected24hr || 0,
        "gateways.$.successfulTransactions": updateDetails.$inc?.successfulTransactions || 0,
        "gateways.$.totalFeeCollected": updateDetails.$inc?.totalFeeCollected || 0,
      },
    };
...
async function updateGatewayBalanceNew(gatewayName, updateDetails) {
  try {
    ...
    const updatedDoc = await updateGatewayDetailsNew(gatewayName,updateDetails)
