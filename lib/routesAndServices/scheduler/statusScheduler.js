const { encryptParameters, getAllPendinTransactions, getAllPendinTransactionsNoLimit } = require("../../appUtils");
const { airpayPayinStatus, airpayPayinStatus2, airpayPayinStatus3, airpayPayinStatus4 } = require("../../gateways/airpay");
...
async function updatePendingTransactionStatus() {
    try {
        const airpay = await getAllPendinTransactionsNoLimit("IN-PROCESS", "airpay");
        ...
        if (airpay && airpay.length > 0) {
           // airpay.reverse(); // Reverse the list before batching
            ...
            const batches = _.chunk(airpay, batchSize); // Split the transactions into batches
...
async function updatePendingTransactionStatus2() {
    try {
        const airpay = await getAllPendinTransactionsNoLimit("IN-PROCESS", "airpay2");
...
async function updatePendingTransactionStatus3() {
    try {
        const airpay = await getAllPendinTransactionsNoLimit("IN-PROCESS", "airpay3");
                        ...
                        try {
                            if (item) {
                                ...
                                if (user && user.length > 0) {
                                    ...
                                    const response = await airpayPayinStatus3(txQuery).catch((e) => {
                                        console.log('Error fetching transaction status:', e);
                                    });
...
async function updatePendingTransactionStatus4() {
    try {
        const airpay = await getAllPendinTransactionsNoLimit("IN-PROCESS", "airpay4");
                        ...
                        try {
                            if (item) {
                                ...
                                if (user && user.length > 0) {
                                    ...
                                    const response = await airpayPayinStatus4(txQuery).catch((e) => {
                                        console.log('Error fetching transaction status:', e);
                                    });
...
async function updatePendingTransactionStatus3New() {
    try {
        const airpay = await getAllPendinTransactions("IN-PROCESS", "airpay3");
...
async function updatePendingTransactionStatus4New() {
    try {
        const airpay = await getAllPendinTransactions("IN-PROCESS", "airpay4");
...
//                                     if (response.TRANSACTIONSTATUS[0] === "200" || response.TRANSACTIONSTATUS[0] === "400") {
//                                         const transaction = await getTransaction(item.toString());
//                                         console.log('tx check',response)
//                                         if (transaction.status === 'success') {
//                                             return;
//                                         }
...
module.exports={
    ...
    updatePendingTransactionStatus3,
    updatePendingTransactionStatus4
